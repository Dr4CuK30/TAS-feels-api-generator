...
export default async (data: FeedsModel) => {
  console.log("|| PROJECT CONFIGURATION ||");
  await loadTypeOrmModule(data);
  await loadEntityConfiguration(data);
};

async function loadTypeOrmModule({ projectName, connection }: FeedsModel) {
...
async function loadEntityConfiguration({ projectName, feedProps }: FeedsModel) {
